...
impl PrimaryToPrimary for PrimaryToPrimaryHandler {
    ...
    async fn send_randomness_partial_signatures(
        ...
        Err(anemo::rpc::Status::new_with_message(
            StatusCode::NotFound,
            "send_randomness_partial_signatures unimplemented",
        ))
    }
    ...
    async fn fetch_headers(
        &self,
        request: anemo::Request<FetchHeadersRequest>,
    ) -> Result<anemo::Response<FetchHeadersResponse>, anemo::rpc::Status> {
        let request = request.into_body();
        if request.max_items == 0 {
            return Ok(anemo::Response::new(FetchHeadersResponse {
                headers: vec![],
            }));
        }
        ...
        let headers = self
            .dag_state
            .read_headers(request.exclusive_lower_bounds, request.max_items)
            .map_err(|e| anemo::rpc::Status::from_error(Box::new(e)))?;
        let response = FetchHeadersResponse { headers };
