...
async fn get_headers_helper(
    ...
    let peers: Vec<NetworkPublicKey> = state
        .committee
        .others_primaries_by_id(state.authority_id)
        .into_iter()
        .map(|(_, _, network_key)| network_key)
        .collect();
    let peer = peers.choose(&mut ThreadRng::default()).unwrap();

    debug!("Starting to get headers");
    let request = Request::new(request).with_timeout(Duration::from_secs(1));
