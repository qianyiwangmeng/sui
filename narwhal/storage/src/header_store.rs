...
impl HeaderStore {
    ...
    /// Retrieves the next header key with round bigger than the given lower bound for the origin.
    /// Returns None if there is no more local header from the origin with bigger round.
    pub fn next_round_header_key(
        &self,
        origin: AuthorityIdentifier,
        lower_bound: Round,
    ) -> StoreResult<Option<HeaderKey>> {
        let index_key = (origin, lower_bound + 1, HeaderDigest::default());
        if let Some(((name, round, digest), _)) = self
            .header_key_by_author
            .unbounded_iter()
            .skip_to(&index_key)?
            .next()
        {
            if name == origin {
                return Ok(Some(HeaderKey::new(round, name, digest)));
            }
        }
        Ok(None)
    }
