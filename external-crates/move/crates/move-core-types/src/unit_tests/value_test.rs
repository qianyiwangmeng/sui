...
fn enum_deserialization_vec_option_runtime_layout_equiv() {
    ...
    println!("vec_ser: {:?}", vec_ser);
    println!("enum_ser: {:?}", enum_ser);
    ...
    R::MoveValue::simple_deserialize(&enum_ser, &enum_layout).unwrap();
    R::MoveValue::simple_deserialize(&vec_ser, &enum_layout).unwrap();

    assert_eq!(
        vec_option.simple_serialize().unwrap(),
        enum_option.simple_serialize().unwrap()
    );
}
